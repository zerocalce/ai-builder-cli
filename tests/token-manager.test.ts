import { TokenManager, TokenRecord } from '../src/utils/token-manager';
...
describe('TokenManager timing and refresh behavior', () => {
  ...
  it('emits warning at ~75% and calls refresh callback', async () => {
    const logs: string[] = [];
    const logger: any = {
      debug: jest.fn((m: any) => logs.push(`debug:${m}`)),
      info: jest.fn((m: any) => logs.push(`info:${m}`)),
      warn: jest.fn((m: any) => logs.push(`warn:${m}`)),
      error: jest.fn((m: any) => logs.push(`error:${m}`))
    };
    ...
    const tm = new TokenManager(logger, refreshCb);
    ...
    tm.on('refreshed', refreshedHandler as any);
    tm.on('warning', warningHandler as any);
  ...
  it('expires token and emits expired when no refresh available', async () => {
    ...
    const tm = new TokenManager(logger);

    const expiredHandler = jest.fn();
    tm.on('expired', expiredHandler as any);
  ...
  it('handles refresh callback throwing an error', async () => {
    ...
    const tm = new TokenManager(logger, badRefresh);
  ...
  it('refreshNow invokes the refresh callback immediately', async () => {
    const logger: any = { debug: jest.fn(), info: jest.fn(), warn: jest.fn(), error: jest.fn() };
    const refreshedToken: TokenRecord = { value: 'now-token', issuedAt: new Date(), expiresIn: 60 };
