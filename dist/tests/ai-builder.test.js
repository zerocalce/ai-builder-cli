...
const project_manager_1 = require("../src/core/project-manager");
const deployment_engine_1 = require("../src/core/deployment-engine");
const config_manager_1 = require("../src/core/config-manager");
const logger_1 = require("../src/utils/logger");
const interface_1 = require("../src/cli/interface");
...
describe('AI Builder Core Components', () => {
    ...
    let configManager;
    let projectManager;
    let deploymentEngine;
    let cli;
    let testDir;
    beforeEach(async () => {
        // Setup test environment
        testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ai-builder-test-'));
        logger = new logger_1.Logger({ level: 'debug', format: 'pretty' });
        configManager = new config_manager_1.ConfigManagerImpl(logger, testDir);
        projectManager = new project_manager_1.ProjectManagerImpl(logger, testDir);
        deploymentEngine = new deployment_engine_1.DeploymentEngineImpl(logger);
        cli = new interface_1.CLIInterface(logger);
    });
    afterEach(async () => {
    ...
    describe('ProjectManager', () => {
        describe('createProject', () => {
            ...
            it('should throw error for invalid project name', async () => {
                await expect(projectManager.createProject('express-api', 'invalid name!', testDir)).rejects.toThrow('Invalid project name');
            });
            it('should throw error if directory already exists', async () => {
                const projectName = 'existing-project';
                const projectPath = path.join(testDir, projectName);
                await fs.promises.mkdir(projectPath, { recursive: true });
                await expect(projectManager.createProject('express-api', projectName, testDir)).rejects.toThrow('Directory already exists');
            });
        });
    ...
    describe('DeploymentEngine', () => {
        describe('deploy', () => {
            ...
            it('should handle deployment failures', async () => {
                ...
                jest.spyOn(deploymentEngine, 'buildProject').mockRejectedValueOnce(new Error('Build failed'));
...
describe('AI Builder Integration Tests', () => {
    ...
    describe('Error Handling', () => {
        it('should handle build failures gracefully', async () => {
            ...
            // Mock build failure
            jest.spyOn(projectManager, 'executeCommand').mockRejectedValueOnce(new Error('Build command failed'));
            const buildResult = await projectManager.buildProject(project);
...
// Helper functions
async function createMockTemplate(name, testDir) {
...
async function createMockProject(name, testDir, pm) {
    await createMockTemplate('express-api', testDir);
...
function createMockDeploymentTarget(type) {
    return {
        name: type,
        type,
        config: {
