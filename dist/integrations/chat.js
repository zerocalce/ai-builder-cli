...
class ChatInterface extends events_1.EventEmitter {
    ...
    async deployProject(projectName, target) {
        try {
            ...
            const deployment = await this.deploymentEngine.deploy(project, { name: target });
    ...
    async getDeploymentStatus(projectName) {
        try {
            ...
            return `❌ Failed to get deployment status: ${error.message}`;
        }
    }
    async listProjects() {
        try {
    ...
    async rollbackProject(projectName, version) {
        try {
            const project = await this.projectManager.loadProject(projectName);
            const deployments = project.deployments;
...
exports.ChatPluginHooks = ChatPluginHooks;
